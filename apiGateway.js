...
}, proxy(`http://44.204.3.3:${process.env.REGISTER_PORT}`, {
...
app.post('/auth/login', (req, res, next) => {
    next(); // Let the proxy middleware handle it
}, proxy(`http://44.204.3.3:${process.env.LOGIN_PORT}`, {
...
app.use('/userCreate/blog/create', authenticate, (req, res, next) => {
    next(); // Let the proxy middleware handle it
}, proxy(`http://44.204.3.3:${process.env.CREATE_BLOG_PORT}`, {
...
app.use('/adminView/blog/admin/all', authenticate, authRole('admin'), (req, res, next) => {
    next(); // Let the proxy middleware handle it
}, proxy(`http://34.228.81.33:${process.env.ADMIN_VIEW_PORT}`, {
